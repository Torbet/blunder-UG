            ...
            if [ $model == "transformer" ] || [ $model == "convlstm" ]; then
                for evals in "--evals" "--no-evals"; do
                    for times in "--times" "--no-times"; do
                        ...
                        python train.py --model $model --lr $lr --weight-decay $weight_decay $evals $times --limit 10000 --num-moves 40 --save
                    done
                done
            else
                echo "model: $model, lr: $lr, weight_decay: $weight_decay"
                python train.py --model $model --lr $lr --weight-decay $weight_decay --limit 10000 --num-moves 40 --save
            fi
        done
    done
done